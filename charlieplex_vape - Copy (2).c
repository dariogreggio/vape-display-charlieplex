...
void high_isr(void) {
  ...
  BYTE segments;                 // The bitpattern of the current digit		( .GFEDCBA )
	...
	if(INTCONbits.TMR0IF) {				// Timer 0
	  ...
	  switch(currentdigit) {
	    ...
	    case 3:		// qua ci sono gli extra/simboli
				...
				TRISA &= ~CHARLIE_1_A_VAL;
			  if(segments & 0b00000001) 		// batteria
					TRISA &= ~CHARLIE_5_A_VAL;
			  if(segments & 0b00000010) 		// goccia
					TRISA &= ~CHARLIE_4_A_VAL;
			  if(segments & 0b00000100) 		// %
					TRISA &= ~CHARLIE_3_A_VAL;
	      break;
...
// #define TEST_SEGMENTI
		...
		LATA= 0b00000001;
		Delay_ms(2000);
		TRISA=0b00001110;		// batteria
		Delay_ms(1000);
		TRISA=0b00010110;		// goccia
		Delay_ms(1000);
		TRISA=0b00011010;		// %
		Delay_ms(1000);
		TRISA=0b00011100;		// (nulla)
		Delay_ms(1000);

		TRISA=0;
		LATA= 0b00000010;		//
		Delay_ms(2000);
		TRISA=0b00001101;		// 3E E-3
		Delay_ms(1000);
		TRISA=0b00010101;		// 1C C-1 
		Delay_ms(1000);
		TRISA=0b00011001;		// 1B B-1
		Delay_ms(1000);
		TRISA=0b00011100;		// 2G G-2
		Delay_ms(1000);

		TRISA=0;
		LATA= 0b00000100;
		Delay_ms(2000);
		TRISA=0b00001011;		// 3C C-3
		Delay_ms(1000);
		TRISA=0b00010011;		// 2A A-2
		Delay_ms(1000);
		TRISA=0b00011001;		// 2C C-2
		Delay_ms(1000);
		TRISA=0b00011010;		// 2F F-2
		Delay_ms(1000);

		TRISA=0;
		LATA= 0b00001000;
		Delay_ms(2000);
		TRISA=0b00000111;		// 3A A-3
		Delay_ms(1000);
		TRISA=0b00010011;		// 2B B-2
		Delay_ms(1000);
		TRISA=0b00010101;		// 2D D-2
		Delay_ms(1000);
		TRISA=0b00010110;		// 2E E-2
		Delay_ms(1000);

		TRISA=0;
		LATA= 0b00010000;
		Delay_ms(2000);
		TRISA=0b00000111;		// 3B B-3
		Delay_ms(1000);
		TRISA=0b00001011;		// 3D D-3
		Delay_ms(1000);
		TRISA=0b00001101;		// 3F F-3
		Delay_ms(1000);
		TRISA=0b00001110;		// 3G G-3
		Delay_ms(1000);
...
ResCnt:
		...
		// Display 100 for one second
		digits[0]=table_7seg[1+1];
		digits[1]=table_7seg[0+1];
		digits[2]=table_7seg[0+1];
		digits[3]=0;
		Delay_ms(1000);
		// Display 75 for one second
		digits[0]=0b00000000;
		digits[1]=0b00000111;
		digits[2]=0b01101101;
		digits[3]=0;
